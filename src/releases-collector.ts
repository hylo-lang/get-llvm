...
export function getArchitecturePlatform(): string {
  if (process.platform === "linux" && process.arch === "arm64") return linuxArmPlatform;
  else if (process.platform === "win32" && process.arch === "arm64") return windowsArmPlatform;
...
export interface MostRecentVersion {
  mostRecentVersion: semver.SemVer | null;
}
...
export class ReleasesCollector {
  ...
  public constructor(
    private map: CatalogType,
    private mostRecentReleases: MostRecentReleases,
    private filters: ReleaseFilter[],
  ) {}
            ...
            try {
              ...
              if (version) {
                ...
                for (const key of ReleasesCollector.versionSelectors) {
                  ...
                  if (v == null) this.mostRecentReleases.set(key.releaseKey, new Map());

                  const latest = this.mostRecentReleases.get(key.releaseKey)?.get(filter.platform);
                  const isCurrentVersionPrerelease =
                    (version.prerelease?.length ?? 0) > 0 ? true : false;
                  if (
                    key.prereleaseAccepted === isCurrentVersionPrerelease &&
                    (!latest ||
                      (latest.mostRecentVersion &&
                        semver.compare(version, latest.mostRecentVersion) >= 0))
                  ) {
                    this.mostRecentReleases
                      .get(key.releaseKey)
                      ?.set(filter.platform, currentVersion);

                    // Ensure existence of the instance for the given key.
                    let v = this.map[key.releaseKey];
                    v ??= this.map[key.releaseKey] = {};
                    v[filter.platform] =
                      this.map[currentVersion.mostRecentVersion.version][filter.platform];
                  }
                }
              }
            } catch (err: any) {
