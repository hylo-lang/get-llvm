...
export class ToolsGetter {
  ...
  async verifyLlvmConfigOnPath() {
    return core.group(`Verifying llvm-config is on PATH`, async () => {
      ...
      if(trim(llvmConfigVersion) !== this.llvmVersion) {
        throw new Error(`llvm-config on PATH has a version mismatch: expected ${this.llvmVersion}, got ${trim(llvmConfigVersion)}`);
      }
    });
  }

  async verifyLLVMConfigVersionInDirectory(llvmBin: string) {
    return core.group(`Verifying llvm-config in `, async () => {
      const llvmConfigPath = path.join(llvmBin, "llvm-config");
      core.info(`Actual path to llvm-config is: '${llvmConfigPath}'`);
      
      const llvmConfigVersion = await execSync(`"${llvmConfigPath}" --version`, {
        encoding: "utf8",
      });
      core.info(`llvm-config version is: '${llvmConfigVersion}'`);
      const versionTrimmed = llvmConfigVersion.trim();
      if(versionTrimmed !== this.llvmVersion) {
        throw new Error(`llvm-config version mismatch: expected ${this.llvmVersion}, got ${versionTrimmed}`);
      }
    });
  }
